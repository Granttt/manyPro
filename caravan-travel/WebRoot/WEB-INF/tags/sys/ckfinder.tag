...
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		for(var i=0; i<files.length; i++){//<c:if test="${type eq 'thumb' || type eq 'images'}}">
		...
		<%if(type == "images") {%>
			for (var i=0; i<urls.length; i++){
				if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
					var tempurl = urls[i];
					li = "<li><img src=\""+tempurl+"\" url=\""+tempurl+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
					li = "<li><a href=\""+tempurl+"\" url=\""+tempurl+"\" target=\"_blank\">"+decodeURIComponent(tempurl.substring(tempurl.lastIndexOf("/")+1))+"</a>";//</c:if>
