...
<head>
	...
	//触发打开事件
	  /*  alert(catid); */
	   if(catid=='f3f8539acc2e4af484f2de9db4eb0875' || catid=='fb4077c6e9b943688af388c078d49510' || catid=='c1f948b1f6534654a7c0450fc814f0f8' || catid=='0e037203e9744b7083e376189955ab5b'){
	   		alert("源文件不可删除,只可修改！")
	   }
	   else{
	   		el.click();
...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="cms:article:edit"><li><a href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">${not empty noAdd?'':'文章添加'}</a></li></shiro:hasPermission>
	</ul>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>栏目</th><th>标题</th><th>权重</th><!-- <th>点击数</th> --><th>发布者</th>
		<c:if test="${article.category.id eq '12b33a3f104046ea819314e779f3315f' || category.parent.id eq '24'}">
			<th>首页推荐</th>
		</c:if>
		<th>发布时间</th><th>更新时间</th><th>操作</th></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="article">
			<tr>
				...
				<c:if test="${article.category.id eq '12b33a3f104046ea819314e779f3315f' || category.parent.id eq '24'}">
					<td>${fns:getDictLabel(article.homeView, 'yes_no', '否')}</td>
				</c:if>
				...
				<td>
					<shiro:hasPermission name="cms:article:edit">
	    				...
	    				<shiro:hasPermission name="cms:article:audit">
							<%-- <a href="javascript:test('${article.id}');" onclick="return confirmx('确认要${article.delFlag ne 0?'发布':'删除'}该文章吗？', this.href)" >${article.delFlag ne 0?'发布':'删除'}</a> --%>
							...
							<%-- <a href="${ctx}/cms/article/delete?id=${article.id}${article.delFlag ne 0?'&isRe=true':''}&categoryId=${article.category.id}" id='${article.id}'></a> --%>
						</shiro:hasPermission>
					</shiro:hasPermission>
				</td>
			</tr>
