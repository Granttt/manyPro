...
public class CKFinderConnectorServlet extends ConnectorServlet {
	...
	Logger log = Logger.getLogger(CKFinderConnectorServlet.class);
	...
	private void prepareGetResponse(final HttpServletRequest request,
		...
		if ("Init".equals(command)){
			...
			if (startupPath!=null){
				...
				if (ss.length==2){
					String realPath = Global.getPhysicalUploadPath() + Global.USERFILES_BASE_URL + principal + "/" + ss[0] + ss[1];
		...
		// 快捷上传，自动创建当前文件夹，并上传到该路径
			...
			String realPath = Global.getPhysicalUploadPath() + Global.USERFILES_BASE_URL
