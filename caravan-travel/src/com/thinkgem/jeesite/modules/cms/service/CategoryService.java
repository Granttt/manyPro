...
//				list.addAll(0, dao.find(dc));
	...
	public List<Category> findAllCategory(List<Category> topCategory){
		
		
		for(Category category:topCategory){
			List<Category> childrens= categoryDao.findOnlyParentId(category.getId());
			
			if(childrens.size()==0){
				category.setChildList(childrens);
			}else{
				category.setChildList(childrens);
				findAllCategory(childrens);
			}
		}
...
/*	
	public List<Category> findAll(List<Category> parent){
		
		for (Category category : parent) {
			List<Category> childrens = categoryDao.findByPid(category.getId());
			if(childrens.size()==0){
				category.setChildList(childrens);
			}else{
				category.setChildList(childrens);
	...
	public List<Category> getTopCategory(String id) {
		// TODO Auto-generated method stub
		List<Category> topCategory= categoryDao.findOnlyByParentId(id);
		return topCategory;
	}
...
//			}
	...
	public Category findParentByChildId(String categoryId) {
		// TODO Auto-generated method stub
		String parentId=categoryDao.findParentIdByChildId(categoryId);
		Category category=categoryDao.get(parentId);
		return category;
	}
}
